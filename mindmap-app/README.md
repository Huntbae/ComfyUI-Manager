...
| **[07. 실측 결과](docs/07-corpus-measurement.md)** | 📊 **실제 `.xmind` 6개(270 토픽) 분석 — 스키마 확정, 왕복 게이트 통과** |
...
- **핵심 기술 결정**: 원본 `.xmind` ZIP을 통째로 보관하고, 편집한 필드만 덮어쓰는
  **패치형 저장(preserve-and-patch)** 방식을 채택한다. 파싱하지 못한 필드도 그대로 살아남는다.
  → **실제 파일 6개로 검증 완료. 무편집 왕복이 바이트 단위로 동일하다.**
- **배포 형태**: 웹(브라우저 로컬 파일 편집) → Tauri 데스크톱 → 협업 서버 순으로 확장.

## 실측 검증 현황 (Phase 0)

실제 Xmind로 작성된 `.xmind` **6개(270 토픽)**를 분석했다. [상세](docs/07-corpus-measurement.md)

| 항목 | 결과 |
|---|---|
| 무편집 왕복 무손실 | ✅ **6/6 통과** |
| `content.json` 바이트 동등 | ✅ **6/6 통과** (예상보다 강한 보증) |
| naive 파서(기존 OSS) 유실량 | ❌ **131개 데이터 포인트** — 플로팅 토픽 3, 관계선 18, 이미지 1 포함 |
| 스키마 확정 | detached · image · position · customWidth · attributedTitle · titleUnedited · relationships |
| 미확정 | 마커 · 경계선 · 요약 · 말풍선 (**해당 기능을 쓴 샘플 필요**) |

### 실측으로 드러난 함정 4가지

1. **`content.xml`은 콘텐츠가 아니라 경고 스텁이다.** modern 파일에도 들어 있고
   6개 파일 MD5가 전부 동일하다. 이걸 먼저 읽는 파서는 사용자 맵 대신 "경고"를 연다.
...
3. **`attributedTitle`이 `title`과 텍스트를 이중 저장한다.** 하나만 고치면
   XMind에서 옛 글자가 보인다. `titleUnedited` 플래그까지 합쳐 **텍스트 편집은 3개 필드를 함께** 다뤄야 한다.
4. **`image`는 `src` 외 전부 선택 필드다.** `width`/`height`를 필수로 가정하면 5건 중 3건이 깨진다.
