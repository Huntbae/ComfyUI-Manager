...
- 백업 드라이브를 훑을 때 타임머신(`Backups.backupdb`)·앱 번들·캐시는 건너뛴다.
  사진 라이브러리(`.photoslibrary`)는 **원본 폴더만** 본다 — `derivatives`는 같은
  사진의 축소본이라 크기가 달라 내용 지문으로도 안 걸러지고, 그대로 두면
  같은 사진이 여러 편에 실린다.
...
- 현재 원고는 30편(`articles/`) — **v6 「기획자가 개발한 전기차 사이클카트」**.
  저자 회고록 원고를 1차 자료로 삼았다. 구성과 확정 사실은 `SERIES.md` 참고.
- **원고에서 의도적으로 뺀 것이 있다** — 특정 공무원·기관과의 갈등 서사, 협력사 실명,
  원고가 ⚠️[구술 필요]로 표시한 미확인 항목, 미확정 수치(주행거리 120km 등).
  이 원칙을 바꾸지 말 것. 이유는 `SERIES.md` 에 적었다.
- 칼리 확정 제원: 차명 NEWTRO M / 형식 NMCK01 / 사륜형 이륜자동차 1인승 /
  제원관리번호 23-XCR-00001 / 공인시험 33종 / 전장 2,495mm / 최고속도 60km/h(법규 상한) /
  최고출력 11kW / 구동계 72V / 축전지 73.8V 86.4Ah 교환식.
- **칼리와 마실카를 끝까지 구분한다.** 1인승은 칼리, 인구소멸지역 마을버스는 마실카.
  마실카는 아직 도로에 없다.
- v1~v4는 `articles_v1/` ~ `articles_v4/`에 보관되어 있고 큐에서 빠져 있다.
  되돌리려면 `mv articles_vN/*.txt articles/`.
  v4(사이클카트 25편)는 내용이 정확하지만 검색 수요가 사실상 0이라 뺐다. 1편만 게시됐다.
